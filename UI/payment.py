...
def booking_window(match_info,matchID,audienceID):
    ...
    booking_win = ctk.CTk()
    booking_win.attributes("-fullscreen", True)
    booking_win.bind("<Escape>", lambda e: booking_win.attributes("-fullscreen", False))    
    booking_win.title("Confirm Booking")
    ctk.set_appearance_mode("dark")
    ctk.set_default_color_theme("blue")
    booking_win.configure(fg_color="#0D1B2A")


    def confirm_booking(match_info,quantity,ticket_price):
        try:
            ...
            if not 1<=quantity<=10:
                ...
                if seats_available < quantity:
                    ...
                    confirm = CTkMessagebox(
                        ...
                        message=f"You are about to book a ticket of Rs. {total_amount:.2f} for {quantity} ticket(s).\nDo you want to continue?",
                        icon="question",
                        option_1="Yes",
                        option_2="Cancel"
                    )
                    ...
                    if confirm.get() == "Yes":
                        ...
                        if success:
                            ...
                            if filename:
                                ...
                                booking_win.destroy()
                                audience_dashboard_window(audienceID)
                        else:
    ...
    frame = ctk.CTkFrame(booking_win, fg_color="#1A1A2E")
    frame.pack(pady=30, padx=30, fill="both", expand=True)

    back_button = ctk.CTkButton(booking_win, text="← Back", width=80, command= lambda: (booking_win.destroy(),audience_dashboard_window(audienceID)))
    back_button.place(x=40, y=40)  

    heading_ = ctk.CTkLabel(frame, text="Booking", font=("Bebas Neue", 28, "bold"), text_color="white")
    ...
    def calculateTotal(quantity,price):
        try:
            quantity=int(quantity)
            if not 1<=quantity<=10:
                total_price_label.configure(text="Tickets quantity must be between 1-10",text_color="#FF0000")
            else:
                total_price_label.configure(text=f"Total Amount: Rs. {(quantity * price):.2f}",text_color="#00FFAA")   
        except ValueError:
            total_price_label.configure(text="Please enter a valid quantity.",text_color="#FF0000")
    ...
    confirm_btn = ctk.CTkButton(frame, text="Confirm Booking",command= lambda: (confirm_booking(match_info,ticket_entry.get(),ticket_price)))
